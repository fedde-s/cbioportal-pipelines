...
public class ClinicalDataConverterImpl extends ConverterBaseImpl implements Converter
{
	...
	private void combineColumns(DataMatrix dataMatrix)
	{
		int headerIndex = -1;
		List<String> processedColumnHeaders = new ArrayList<String>();
		for (String externalColumnHeader : dataMatrix.getColumnHeaders()) {
			++headerIndex;
			if (processedColumnHeaders.contains(externalColumnHeader)) {
        		logMessage(LOG, "duplicate column header, calling combineColumns(): " + externalColumnHeader);
				combineColumns(dataMatrix, processedColumnHeaders.indexOf(externalColumnHeader), headerIndex);
				dataMatrix.ignoreColumn(headerIndex, true);
				processedColumnHeaders.add("PLACEHOLDER_HEADER");
			}
			else {
				processedColumnHeaders.add(externalColumnHeader);
			}
		}
	}

	private void combineColumns(DataMatrix dataMatrix, int targetColumnIndex, int duplicateColumnIndex)
	{
		List<String> targetColumnData = dataMatrix.getColumnData(targetColumnIndex);
		List<String> duplicateColumnData = dataMatrix.getColumnData(duplicateColumnIndex);
		for (int lc = 0; lc < dataMatrix.getNumberOfRows(); lc++) {
			String targetColumnValue = targetColumnData.get(lc);
			String duplicateColumnValue = duplicateColumnData.get(lc);
			if (!targetColumnValue.equals(duplicateColumnValue) &&
				MissingAttributeValues.has(targetColumnValue) &&
				!MissingAttributeValues.has(duplicateColumnValue)) {
				logMessage(LOG, "combineColumns, merging values (id, target, duplicate): " + dataMatrix.getColumnData(0).get(lc) + ", " + targetColumnValue + ", " + duplicateColumnValue);
				targetColumnData.set(lc, duplicateColumnValue);
			} 
		}

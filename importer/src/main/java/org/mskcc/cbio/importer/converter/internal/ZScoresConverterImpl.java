	...
	public void createStagingFile(PortalMetadata portalMetadata, CancerStudyMetadata cancerStudyMetadata,
		...
		if (dependencies.length == 0) {
			if (LOG.isErrorEnabled()) {
				LOG.error("createStagingFile(), dataMatrices.length == 0, aborting...");
			}
			return;
		}

		// we assume dependency staging files have already been created, get paths to dependencies
		DatatypeMetadata[] dependenciesMetadata = new DatatypeMetadata[dependencies.length];
		try {
                    for (int i=0; i<dependencies.length; i++) {
			dependenciesMetadata[i] = (DatatypeMetadata)config.getDatatypeMetadata(dependencies[i]).iterator().next();
                    }
		}

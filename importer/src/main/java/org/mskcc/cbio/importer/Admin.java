...
public class Admin implements Runnable {
	...
	private int updateStudyData(String portal, String updateWorksheet, String sendNotification) throws Exception
	{
		...
		Map<String,String> mskPropertyMap = new HashMap<String,String>();
		mskPropertyMap.put(CancerStudyMetadata.MSK_PORTAL_COLUMN_KEY, "");
		Map<String,String> triagePropertyMap = new HashMap<String,String>();
		triagePropertyMap.put(CancerStudyMetadata.TRIAGE_PORTAL_COLUMN_KEY, "");
		...
		for (CancerStudyMetadata cancerStudyMetadata : config.getCancerStudyMetadata(portal)) {
			if (cancerStudyMetadata.isUpdateStudyDelete()) {
				...
				try {
					...
					errorMap.put(cancerStudyMetadata.getStableId(), sw.toString());
				}
			}
			if (updateWorksheetBool) {
				if (portal.equals(PortalMetadata.MSK_AUTOMATION_PORTAL)) {
					config.updateCancerStudyAttributes(cancerStudyMetadata.getStudyPath(), mskPropertyMap);
				}
				else if (portal.equals(PortalMetadata.TRIAGE_PORTAL)) {
					config.updateCancerStudyAttributes(cancerStudyMetadata.getStudyPath(), triagePropertyMap);
				}
			}
		}
		if (sendNotificationBool && (!cancerStudiesUpdated.isEmpty() || !cancerStudiesRemoved.isEmpty() || !errorMap.isEmpty())) {
			sendNotification(portal, cancerStudiesUpdated, cancerStudiesRemoved, errorMap);
		}
	...
	private void sendNotification(String portal, List<String> cancerStudiesUpdated,
	                              List<String> cancerStudiesRemoved, Map<String,String> errorMap)
	{
		Config config = (Config)getBean("config");
		SimpleMailMessage message = null;
		if (portal.equals(CancerStudyMetadata.MSK_PORTAL_COLUMN_KEY) ||
			portal.equals(CancerStudyMetadata.MSK_IMPACT_PORTAL_COLUMN_KEY)) {
		...
		if (!errorMap.isEmpty()) {
			body += "\n\n" + "The following studies had errors during import:\n\n";
			for (String cancerStudyStableId : errorMap.keySet()) {
				body += cancerStudyStableId + "\n" + errorMap.get(cancerStudyStableId) + "\n\n";
			}
		}
		msg.setText(body);
		try {
